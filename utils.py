...
"""Save checkpoint"""
def save_cpk(model,random,scheduler,optimizer,epoch,early_stopping,_metric_val,best_ckc_metric,best_ckc_metric_te, output_path):
    random_state = {
    ...
    ckp = {
        ...
        'random': random_state,
        'ckc_metric': _metric_val,
        'val_best_metric': best_ckc_metric,
        'te_best_metric': best_ckc_metric_te
    }
    # Save last model
    torch.save(ckp, output_path)
    #print(f"[Checkpoint] Latest model saved at: {output_path}")
...
"""Cuda configuration"""
def config_cuda(no_determ, no_deter_algo):
