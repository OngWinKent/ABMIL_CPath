...
class InnerAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 20260819 WK Ong: Fix nan loss error
        attn = (q @ k.transpose(-2, -1)).clamp(min=-50.0, max=50.0)
        ...
        # 20260819 WK Ong Fix nan loss error
        attn = self.softmax(attn.float()).to(q.dtype)
...
class CrossRegionAttntion(nn.Module):
    ...
    def forward(self,x,return_attn=False):
        ...
        # 20260819 WK Ong: Fix nan loss, dynamic epsilon based on precision
        '''
        eps = 1e-5 if logits.dtype == torch.float16 else 1e-8
        denom = (logits_max.unsqueeze(-1) - logits_min.unsqueeze(-1)).clamp(min=eps)
        dispatch_weights_mm = (logits - logits_min.unsqueeze(-1)) / denom
        '''
        logits_fp32 = logits.float()
        logits_min = logits_fp32.min(dim=-1, keepdim=True)[0]
        logits_max = logits_fp32.max(dim=-1, keepdim=True)[0]
        denom = (logits_max - logits_min).clamp(min=1e-5)
        dispatch_weights_mm = ((logits_fp32 - logits_min) / denom).to(logits.dtype)
